    ...
    if(len(tmp_data)!=0):
      for j in xrange(len(tmp_data)):
        ...
        if(img.size[0]>img.size[1]):
          scale = float(crop_size)/float(img.size[1])
          img = np.array(cv2.resize(np.array(img),(int(img.size[0] * scale + 1), crop_size))).astype(np.float32)
        else:
          scale = float(crop_size)/float(img.size[0])
          img = np.array(cv2.resize(np.array(img),(crop_size, int(img.size[1] * scale + 1)))).astype(np.float32)
